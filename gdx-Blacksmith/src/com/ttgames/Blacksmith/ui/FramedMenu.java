...
public class FramedMenu {
	...
	// fontHeight and rows are used to estimate how tall our table is
	// (frustratingly,
	...
	public FramedMenu(Blacksmith game, Skin skin, float preferredWidth, float preferredHeight) {
		this(game, skin, preferredWidth, preferredHeight, null);
	}

	public FramedMenu(Blacksmith game, Skin skin, float maxWidth, float maxHeight,
			FramedMenu parent) {
		this.game = game;
		this.skin = skin;
	...
	// Adds a button to the menu, with a secondary label (like MP cost) aligned
	// to the right
	public void addButton(String label, String secondaryLabel,
			ChangeListener listener, boolean active) {
		LabelStyle style;
		if (active)
			style = skin.get(LabelStyle.class);
		else
			style = skin.get("inactive", LabelStyle.class);
	...
	// Wipe all the buttons off, and remove widgets from stage (and reset row
	// count)
	public void clear() {
		table.clear();
